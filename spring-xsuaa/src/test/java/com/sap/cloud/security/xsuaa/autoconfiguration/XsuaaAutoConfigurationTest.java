...
public class XsuaaAutoConfigurationTest {
	...
	public final void test_configurationIsInactive_if_noJwtOnClasspath() {
		...
		// Note: this is a safety check, to make sure that the test below succeeds
		// really
		...
		// check that the beans are NOT there, if Jwt.class is filtered out of the
		// classpath.
		contextRunner.withClassLoader(new FilteredClassLoader(Jwt.class)) // make sure Jwt.class is not on the
																			// classpath.
				.run((context) -> {
	...
	public final void test_userConfigurationsCanOverrideDefaultBeans() {
		contextRunner.withUserConfiguration(UserConfiguration.class)
				.run((context) -> {
					...
					XsuaaTokenFlows expectedCustomTokenFlows = customConfig.userDefinedXsuaaTokenFlows(
							customConfig.userDefinedXsuaaTokenFlowRestTemplate(),
							customConfig.userDefinedXsuaaTokenDecoder());
					VariableKeySetUriTokenDecoder expectedCustomTokenDecoder = customConfig
							.userDefinedXsuaaTokenDecoder();
					...
					assertThat(context.getBean(VariableKeySetUriTokenDecoder.class))
							.isSameAs(expectedCustomTokenDecoder);
